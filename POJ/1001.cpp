...
std::ostream& operator << (std::ostream& out, const bint& a) {
	...
	for (int i = tmp.len - 1; ~i; i--) 
		if (tmp.s[i] || i == 0) {
			for (int j = i; ~j; j--) out << char(tmp.s[j] + '0'); break;
		}
	return out;
}
...
int main() {
	...
	while (cin >> str >> n) {
		...
		bint r(str.c_str()), ans = 1;
		for (int i = 0; i < n; i++)
			ans = ans * r;
		pos = pos * n;
		ostringstream sout;
		sout << ans;
		string sans = sout.str();
		if (pos == 0) {
			cout << sans << endl;
		} else if (sans.length() < pos) {
			putchar('.');
			for (int i = 0; i < pos - sans.length(); i++)
				putchar('0');
			cout << sans << endl;
		} else {
			for (int i = 0; i < sans.length() - pos; i++)
				putchar(sans[i]);
			putchar('.');
			for (int i = sans.length() - pos; i < sans.length(); i++)
				putchar(sans[i]);
			putchar('\n');
		}
	}
