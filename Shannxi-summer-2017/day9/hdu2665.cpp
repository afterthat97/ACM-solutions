		...
		for (int now = 1, cnt = 1; now <= cnt; now++) {
			if (que[now].l == que[now].r || que[now].ql > que[now].qr) {
				for (int i = que[now].ql; i <= que[now].qr; i++)
					ans[flr[que[now].f][i].id] = que[now].l;
				continue;
			}
			...
			for (int i = que[now].ql; i <= que[now].qr; i++) {
				int kth = sum(flr[que[now].f][i].r) - sum(flr[que[now].f][i].l - 1);
				if (flr[que[now].f][i].k <= kth)
					flr[1 - que[now].f][++flag1] = flr[que[now].f][i];
				else {
					flr[que[now].f][i].k -= kth;
					flr[1 - que[now].f][--flag2] = flr[que[now].f][i];
				}
			}
			que[cnt + 1].l = que[now].l;
			que[cnt + 1].r = mid;
			que[cnt + 1].f = 1 - que[now].f;
			que[cnt + 1].ql = que[now].ql;
			que[cnt + 1].qr = flag1;
			
			que[cnt + 2].l = mid + 1;
			que[cnt + 2].r = que[now].r;
			que[cnt + 2].f = 1 - que[now].f;
