...
void init() {
	...
	for (int i = _n; i < _n + n; i++) { 
		tree[i].sumv = tree[i].addv = b[i - _n + 1];
	}
...
int main() {
	...
	for (int i = 0; i < m; i++) {
		...
		if (cho == 1) {
			a[x] = y;
			for (int k = max(x - 1, 2); k <= min(x + 1, n - 1); k++)
				if (a[k] > a[k - 1] && a[k] > a[k + 1]) {
					if (b[k] == 0) update(1, 1, _n, k, k, 1);
					b[k] = 1;
				} else {
					if (b[k] == 1) update(1, 1, _n, k, k, -1);
					b[k] = 0;
				}
		} else {
			printf("%lld\n", query(1, 1, _n, x, y, 0));
