...
public:
    ...
    bint(const bint& a) { len = a.len; memcpy(s, a.s, sizeof(char) * len); }
    bint& operator = (const bint& a) { 
        len = a.len; memcpy(s, a.s, sizeof(char) * len); return *this; 
    }
    int to_int() {
...
int main() {
    int n; a[1] = a[2] = a[3] = a[4] = 1;
    for (int i = 5; i < 8000; i++)
        a[i] = a[i - 1] + a[i - 2] + a[i - 3] + a[i - 4];
    while (scanf("%d", &n) == 1) cout << a[n] << endl;
    return 0;
}
